...
export function DevicePanel({ id, focusTab, focusKey }: { id: string; focusTab?: string; focusKey?: number }) {
  ...
  const externalScan = useMutation({
    mutationFn: () => api.post<{ reachable: number; tested: number; public_ip: string }>(`/devices/${id}/external-scan`),
        ...
        {tab === "network" && (
          <section className="card" style={{ marginTop: 12 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
              ...
              {canOperate && (device.listen_ports ?? []).some((p) => p.public && p.proto.startsWith("tcp")) && (
                <button className="btn ghost sm" disabled={externalScan.isPending} style={{ marginLeft: "auto" }}
                  title={t("Der Server testet die öffentliche IP von außen auf den öffentlichen TCP-Ports.")}
                  onClick={() => externalScan.mutate()}>🌍 {externalScan.isPending ? t("prüfe…") : t("Von außen prüfen")}</button>
              )}
            </div>
            ...
            {(device.listen_ports ?? []).length === 0 ? (
              ...
              <div className="scroll-list">
                <table className="table">
                  <thead><tr><th>{t("Sichtbarkeit")}</th><th>{t("Proto")}</th><th>{t("Adresse")}</th><th>Port</th><th>{t("Prozess")}</th><th>{t("Von außen")}</th></tr></thead>
                  <tbody>
                    {device.listen_ports!.map((p, idx) => (
                      <tr key={idx}>
                        ...
                        <td className="small">{!p.public || !p.proto.startsWith("tcp") ? <span className="muted">—</span>
                          : p.ext_checked
                            ? (p.ext_reachable ? <span style={{ color: "var(--red)" }}>● {t("erreichbar")}</span> : <span style={{ color: "var(--green)" }}>● {t("blockiert")}</span>)
                            : <span className="muted">{t("ungeprüft")}</span>}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
